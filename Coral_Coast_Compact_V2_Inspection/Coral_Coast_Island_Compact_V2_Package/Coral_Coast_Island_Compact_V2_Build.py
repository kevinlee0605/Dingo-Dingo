...
OUT = Path('/mnt/data/Coral_Coast_Island_Compact_V2_Package')
TEX_DIR = OUT / 'Textures'
OUT.mkdir(parents=True, exist_ok=True)
TEX_DIR.mkdir(parents=True, exist_ok=True)

GLB_PATH = OUT / 'Coral_Coast_Island_Compact_V2.glb'
ZIP_PATH = Path('/mnt/data/Coral_Coast_Island_Compact_V2_Roblox_Ready.zip')
README_PATH = OUT / 'README.txt'
VERIFY_PATH = OUT / 'verification.json'
PREVIEW_PATH = OUT / 'Coral_Coast_Island_Compact_V2_Preview.png'
TOP_PATH = OUT / 'Coral_Coast_Island_Compact_V2_Top.png'
GROUND_PATH = OUT / 'Coral_Coast_Island_Compact_V2_Ground.png'
BUILD_SCRIPT_COPY = OUT / 'Coral_Coast_Island_Compact_V2_Build.py'
...
def palm_parts(x,z,height_cells=7,canopy_radius=2,lean=(0,0)):
    ...
    for y in range(height_cells):
        sx=0 if y<height_cells//2 else dx
        sz=0 if y<height_cells//2 else dz
    ...
    for lx in range(-1,2):
        for lz in range(-1,2): leaf_cells[(lx,1,lz)]='Leaves'
...
verification={
    'asset':'Coral Coast Island Compact Revision',
    'root_name':ROOT_NAME,
...
README=f'''CORAL COAST ISLAND — COMPACT ROBLOX-READY VOXEL REVISION
