...
local function isOceanSplit(instance)
	local normalized = normalize(instance.Name)
	return instance:IsA("Model") and (normalized == "oceanrodsplit" or normalized == "oceanrodsplit2")
end

local function isOldOceanRod(instance)
	if not (instance:IsA("Model") or instance:IsA("BasePart")) then
		return false
	end

	local normalized = normalize(instance.Name)
	return normalized == "oceanrod"
		or normalized == "oceanrod11"
		or normalized == "ocean"
		or normalized == "oceanrodold"
end

local function findOceanSplit()
	for _, container in ipairs({ Workspace, ReplicatedStorage, ServerStorage }) do
		for _, descendant in ipairs(container:GetDescendants()) do
			if isOceanSplit(descendant) then
				return descendant
			end
		end
	end
...
local split = findOceanSplit()
assert(split, "Could not find a Model named OceanRod_Split in Workspace, ReplicatedStorage, or ServerStorage.")

local folder = makeTemplateFolder()
local existingTemplate = folder:FindFirstChild("OceanRod")
if existingTemplate and existingTemplate ~= split then
	existingTemplate:Destroy()
end

local template = split:Clone()
template.Name = "OceanRod"
...
for _, container in ipairs({ Workspace, ReplicatedStorage }) do
	for _, child in ipairs(container:GetDescendants()) do
		if child ~= split and isOldOceanRod(child) then
			child:Destroy()
		end
	end
end

print("[FishyFish] Replaced old Ocean Rod template with OceanRod_Split.")
